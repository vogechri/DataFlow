...
%
function [u,v,w,p,q] = solve_TGV( I1, I2, A, u, v, w, p,q, par, constraints)

[M, N, C] = size(I1);
vv = cat( 1, u(:), v(:), w(:) );clear('w');
y  = cat( 1, p(:), q(:) );clear('p');clear('q');
...
for w=1:par.warps
  ...
  for i=1:par.maxits
    ...
    if exist('constraints', 'var' )
      ...
      y1 = reshape(y(1:end-numC-1), [N*M, 4,3]);
      ys = sum(y1.^2, 2);
...
clear('vv_');
w = reshape(vv(1+N*M*2:end),M,N,4);
% remove constraints before this (last columns)
if exist('constraints', 'var' )
  y(end-numC:end) = [];
end
p = reshape(y(1:N*M*4),M,N,4);
q = reshape(y(1+N*M*4:end), M,N,8);
