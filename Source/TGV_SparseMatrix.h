...
public:
  ...
  eigenMatrixVecor( int _rows, int _cols, Scalar*& _uvw, Scalar*& _pq, int _elements, Scalar*& ids_i, Scalar*& ids_j, Scalar*& ids_v, 
...
//      pq[i+restCons] = cons[i];
  ...
  void createMatrices(Scalar*& ids_i, Scalar*& ids_j, Scalar*& ids_v)
  {
    ...
    tau.resize( pixel );
//    std::copy( normalizerC.begin(), normalizerC.begin()+pixel, tau.begin() );
...
//      else
//        tripletList.push_back ( T( ids_i[i]-1, ids_j[i]-1, 0 ) );

      if ( normalizerC[ ids_j[i]-1 ] > 0 )
        tripletListT.push_back( T( ids_j[i]-1, ids_i[i]-1, ids_v[i]/normalizerC[ ids_j[i]-1 ] ) );
//      else
//        tripletListT.push_back( T( ids_j[i]-1, ids_i[i]-1, 0 ) );

      //      int col = tripletList[i].col();
      //      int row = tripletList[i].row();
      //      Scalar val = tripletList[i].value();
    }
